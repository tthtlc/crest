...
namespace crest {
	...
	UnaryExpr::~UnaryExpr() { delete child_;}

	size_t UnaryExpr::Size() { return child_->Size()+1; }

	void UnaryExpr::AppendVars(set<var_t>* vars) { child_->AppendVars(vars); }

	bool UnaryExpr::DependsOn(const map<var_t,type_t>& vars) { return child_->DependsOn(vars); }
	...
	bool UnaryExpr::IsConcrete() { return false; }
