...
namespace crest {
	...
	CompareExpr::~CompareExpr() { delete left_; delete right_; }

	size_t CompareExpr::Size() { return left_->Size() + right_->Size() + 1; }

	void CompareExpr::AppendVars(set<var_t>* vars) { left_->AppendVars(vars); right_->AppendVars(vars); }

	bool CompareExpr::DependsOn(const map<var_t,type_t>& vars) { return left_->DependsOn(vars) || right_->DependsOn(vars); }
	...
	bool CompareExpr::IsConcrete() { return false; }
