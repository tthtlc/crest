...
namespace crest {
	...
	BinaryExpr::~BinaryExpr() { delete left_; delete right_; }

	size_t BinaryExpr::Size() { return left_->Size() + right_->Size() + 1; }

	void BinaryExpr::AppendVars(set<var_t>* vars) { left_->AppendVars(vars); right_->AppendVars(vars); }

	bool BinaryExpr::DependsOn(const map<var_t,type_t>& vars) { return left_->DependsOn(vars) || right_->DependsOn(vars); }
	...
	bool BinaryExpr::IsConcrete() { return false; }
